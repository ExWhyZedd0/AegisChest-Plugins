...
- **Timed Expiry**: AegisChests automatically expire after a configurable duration (default: 10 minutes), naturally dropping the items on the ground if not collected.
...
1. Download the latest `AegisChest-1.x.x.jar` release.
...
```yaml
# aegischest-duration: The time in seconds before an AegisChest expires.
aegischest-duration: 600
...
*Currently, AegisChest works out-of-the-box for all players. Server operators (OPs) have the bypass ability to open or break any AegisChest.*

## 🛠️ Building from Source

To build this project from source, you need JDK 21+ and Gradle.
